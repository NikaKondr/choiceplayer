...
const ChoicеPlayer = ({store}) => {
	...
	React.useEffect(() => {
		EventManager.addHandler('choicePlayer', value => {
			if (value.type === 'setData') {
				store.setDataPlayers(value.data);
			} else if (value.type === 'changePlayer') {
				store.changePlayer(value.data);
			} else return;
		});
